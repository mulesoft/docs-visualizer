...
* *Customization*: Provides information about the layer to which the node is assigned and the tags assigned to the node. You can xref:(customization)[edit the node's display name] or xref:layers.adoc#[change the layer] and xref:use-tags-in-visualizer.adoc[tags] from this section.
