...
However, due to a potential performance impact, header injection is disabled by default for Mule Version 4.x with Anypoint Monitoring agent released before <date>. You must enable header injection at the server (not app) level.
