...
Due to a potential performance impact, however, header injection is disabled by default for Mule Version 4.x released before <date>.
...
For a standalone Mule deployment using Mule Version 4.x released before <date>, you must enable header injection at the server (not app) level.
