...
However, due to a potential performance impact, header injection is disabled by default for Mule version 4.x with Anypoint Monitoring Agent released before April 6, 2020. Enable header injection at the server (not app) level.
...
Set the property to `false`, as shown in the example to enable header injection, because the property is disabled by default.
