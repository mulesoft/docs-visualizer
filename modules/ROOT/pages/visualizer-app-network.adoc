...
Metrics are also available for connections, or _edges_, in your network. When you select average response time or average throughput, metrics also appear on the related connection. For more information on which metrics are available, see xref:connection-metrics-compatibility.adoc[Supported Connection-Level Metrics].
...
Depending on if the selected metric is compatible, the canvas will also display metrics on the connections. When you select a connection within the graph, Anypoint Visualizer displays the edge’s metrics details in a card.
For more information about which connections support this feature, see xref:connection-level-metrics.adoc[Supported Connection-Level Metrics].
