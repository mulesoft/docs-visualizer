...
(nodes with inbound connections only) are shown at the bottom.
When the `ClientId` is detected for a consumer node, the nodes are grouped by `ClientId`. Otherwise, they are grouped under the *External Traffic* node.
...
If you select multiple nodes, you can also click *Hide selected services* to hide the selected nodes, or you can click *Hide other services* to hide services that are unrelated to the selected nodes.
