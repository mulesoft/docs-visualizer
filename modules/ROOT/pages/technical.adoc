...
Visualizer requires an additional header injection to work properly for standalone Mule and Runtime Fabric deployments. To reduce potential performance impact for an application running in a standalone Mule 4.1.x, the header injection is disabled by default. 
...
Set the following property in either the server (for Mule standalone deployment) or application (for a Runtime Fabric deployment):

----
anypoint.platform.config.analytics.agent.header_injection.disabled=false
----

To reduce potential performance impact for applications running in a standalone Mule, you can do one of the following:
