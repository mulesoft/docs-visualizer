...
Set the following property in either the server (for Mule standalone deployment) or app (for a Runtime Fabric deployment):
...
Reducing performance impact for an app running in a standalone Mule is done at the server level. You can do one of the following:
