...
=== Supported Standalone Mule Runtime Engine (Mule) Versions
...
* If a CloudHub application with the old agent is calling a Runtime Fabric ingress endpoint, you see an extra edge from the CloudHub app to a node with the IP for the controller.
* If a Mule app deployed to Runtime Fabric is calling a Mule app deployed to CloudHub using its DNS, the correct connection will be on the Visualizer canvas, but there will be an additional edge from the External Traffic node to the CloudHub application.
...
[NOTE]
You must enable the header injection at the server, not application, level.
