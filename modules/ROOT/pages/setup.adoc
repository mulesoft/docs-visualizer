= Set up API Visualizer

Configure the roles and permissions required to use API Visualizer and understand what to consider when deploying applications.

== Configure Permissions

To view content in API Visualizer, you must be assigned a role that has one of the following permissions:
...
API Visualizer supports the following deployment types:
...
When you deploy an app you want to view in API Visualizer, ensure that you select a supported version of Mule as the deployment target. See xref:runtime-manager::index.adoc[Runtime Manager] for more information.

After redeploying an app, all metadata collected by API Visualizer is removed for that app. The number of inbound connections for the app node is reset.

== Verify Supported Connectors

API Visualizer can monitor activity for outbound connections on specific connectors.
