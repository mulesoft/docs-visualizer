...
so you must enable it at the server, not application, level.
...
Note that you must set the property to `false` as shown in the example to enable header injection, since the property is `disabled`.
