...
| Yes
| If there is a connection from external traffic to a Mule app and also a connection from a Mule app using Mule version 4.x before <CH Release date with UA 2.2.2> or an -AM extension to the same app, then the Mule-app-to-Mule-app edge metric does not appear; rather, it is averaged into the value shown on the edge from external traffic.

| Mule App to a Mule App
| Yes
| If the source node is a CloudHub app using Mule version 4.x before <CH Release date with UA 2.2.2> or an -AM extension, then metrics do not appear on that connection and the following message appears:
...
| Mule App to a Mule App
| Yes
| If the source node is a Runtime Fabric app using http connector below 1.5.12 on Mule version 4.x before <rtf release date with UA 2.2.2> and you have not enabled the additional header injection, then metrics do not appear on that connection and the following message appears:
