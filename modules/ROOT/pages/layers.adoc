...
When this property is used, the nodes automatically appear in the specified layer, and if the layer doesn't exist it will be automatically created. A node layer can be reassigned through the Runtime Manager UI.
...
. In the Visualizer display, click inside the layer to delete. Do not click a node within the layer.
