...
You can automate adding nodes to a layer by using `anypoint.platform.visualizer.layer=<name>`, either in the application's properties you add at deployment time through the Runtime Manager UI, CI/CD (Maven), or API. This property is an application property, not a server property. When you use this property, nodes automatically appear in the specified layer and, if that layer doesn't exist, it is automatically created.
