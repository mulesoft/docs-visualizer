...
To automate adding layer(s) to a node, use the property `anypoint.platform.visualizer.tags=<name>, <name>`` in the application’s properties file, or at deployment time through the Runtime Manager UI or API. Note that this is an application property, not a server property. You can only use the Runtime Manager UI or Cloudhub deployments.
When you use this property, nodes automatically appear in the specified layer. If the layer doesn't exist, it is created.

[NOTE]
You cannot reassign a node layer using the Anypoint Visualizer UI if you assign it using application property.
