= Troubleshooting API Visualizer

If you encounter an error or an issue while using API Visualizer, you can try these solutions.
...
|`There are no services to show in the selected environments. Update the update your applications' runtime versions so they can appear in API Visualizer.`
|API Visualizer currently supports only applications deployed to CloudHub and standalone Mule runtime engines (Mule) with specific runtimes.
|If you know that you have applications deployed in a supported environment, but are not seeing them in the API Visualizer canvas, update your applications to use the supported Mule version. View a list of xref:setup.adoc[supported Mule runtime engines].
|Apps aren't appearing (and you aren't getting a specific error message).
|API Visualizer only supports specific Mule versions (runtimes).
|If you are not seeing some of your applications in the API Visualizer canvas or in the services list, check your filters. If the filters are not the issue, update your applications to use a different xref:setup.adoc[supported Mule version] for the applications to appear on the canvas. If you have upgraded to a version of Mule 4 that was released after March 22, 2019, xref:monitoring::enable-apps-deployed-to-cloud.adoc[enable monitoring] before the applications and metrics can appear.
|===
...
|While all users have access at no additional cost to API Visualizer and Anypoint Monitoring, your account is first enabled.
|File a support ticket or contact your Customer Success representative and ask for your account to be enabled.
|`Metrics aren't available for this app. Update the update its runtime version.`
|The application is not using a Mule version that supports Anypoint Monitoring metrics.
|For CloudHub applications, use an `-AM` Mule to support Anypoint Monitoring, or upgrade to a Mule 4 version available after March 22, 2019, and then enable the Monitoring agent. View a list of xref:setup.adoc[supported Mule versions].
|===
...
|You need the Visualizer Editor permission to edit in API Visualizer.
|Learn more about the different permissions in API Visualizer and how to configure xref:access-management::roles.adoc#custom-roles[custom roles].
...
When you use Database Connector, API Visualizer does not indicate the type of database, such as Postgres or Oracle, from which the traffic originates. Additionally, API Visualizer cannot differentiate between instances of a database.
...
To avoid high graph cardinality, API Visualizer does not show outbound traffic to IP addresses or localhost.
