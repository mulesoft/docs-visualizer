...
Anypoint Visualizer feature availability depends on the control plane for your Anypoint Platform instance. Data isn’t shared across the separate instances of Anypoint Platform.

[%header,cols="3a,1a,1a,1a,1a"]
|===
|Anypoint Visualizer Feature 
...
| Topology is shown for active apps (versus all apps). Active apps are apps that have inbound and outbound traffic within the past seven days.
| Yes
| No
| Yes
| Yes
...
|External nodes are grouped by ClientId when the ClientId is present in the app_inbound request. 
| Yes
| No
| Yes
| Yes
